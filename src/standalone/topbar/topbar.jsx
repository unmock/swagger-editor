...
export default class Topbar extends React.Component {
    ...
    if (this.hasParserErrors()) {
      if (language === "yaml") {
        const shouldContinue = confirm(
          "Swagger-Editor isn't able to parse your API definition. Are you sure you want to save the editor content as YAML?"
        );
        if (!shouldContinue) return;
      } else {
        return alert(
          "Save as YAML is not currently possible because Swagger-Editor wasn't able to parse your API definiton."
        );
      }
    }
  ...
  saveAsJson = () => {
    ...
    if (this.hasParserErrors()) {
      ...
      return alert(
        "Save as JSON is not currently possible because Swagger-Editor wasn't able to parse your API definiton."
      );
    }
  ...
  saveAsText = () => {
    ...
    console.warn(
      "DEPRECATED: saveAsText will be removed in the next minor version."
    );
    let editorContent = this.props.specSelectors.specStr();
  ...
  hasParserErrors = () => {
    return (
      this.props.errSelectors
        .allErrors()
        .filter(err => err.get("source") === "parser").size > 0
    );
  };
  ...
  getFileName = () => {
    ...
    if (
      this.props.specSelectors.isSwagger2 &&
      this.props.specSelectors.isSwagger2()
    ) {
      return "swagger";
    }
  ...
  getDefinitionVersion = () => {
    ...
    return isOAS3() ? "OAS3" : isSwagger2() ? "Swagger2" : "Unknown";
  };
    ...
    */
    ...
    let makeMenuOptions = name => {
      ...
      return {
        ...
        align: "left",
        toggle: (
          <span className="menu-item" onClick={toggleFn}>
            {name}
          </span>
        ),
      };
    ...
    if (isJson) {
      saveAsElements.push(
        <li>
          <button type="button" onClick={this.saveAsJson}>
            Save as JSON
          </button>
        </li>
      );
      saveAsElements.push(
        <li>
          <button type="button" onClick={this.saveAsYaml}>
        ...
        <div className="topbar">
          <div className="topbar-wrapper">
            ...
            <DropdownMenu {...makeMenuOptions("File")}>
              <li>
                <button type="button" onClick={this.importFromURL}>
                  Import URL
                </button>
              </li>
              <ImportFileMenuItem
                onDocumentLoad={content =>
                  this.props.specActions.updateSpec(content)
                }
              />
              <li role="separator" />
              {saveAsElements}
            ...
            <DropdownMenu {...makeMenuOptions("Edit")}>
              <li>
                <button type="button" onClick={this.convertToYaml}>
                  Convert to YAML
                </button>
              </li>
            ...
            {showServersMenu ? (
              ...
              >
                {this.state.servers.map((serv, i) => (
                  <li key={i}>
                    <button
                      type="button"
                      onClick={this.downloadGeneratedFile.bind(
                        null,
                        "server",
                        serv
                      )}
